...
public class DaoGeneric <T>{
   ...
   /*Caso não exista no banco , salva. Se não,ele Atualiza.
    *Esse método retorna uma entidade.PORÉM, para atualizar um registro
    *é necessário primeiro BUSCAR o registro no banco, para depois atualizar.
	*/
	...
	/*Método responsável por pesquisar um objeto no banco.
	  Primeiro,é pesquisado o chave primaria ,que por sua vez 
	  é armazenado em id.O método find recebe a entidade 
	  passada como parametro com id, retornando um objeto da entidade */
	...
	/*Método responsável por excluir um registro na base de dados.
	 * note que foi usada SQL nativo , para fins de evitar erros e conflitos. 
	 * */
	public void deletarPorId(T entidade) {
		...
		"delete from "+entidade.getClass().getSimpleName().toLowerCase()+
		 " where id ="+ id ; //atribui a sql a string
